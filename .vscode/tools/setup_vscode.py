...
def overwrite_python_analysis_extra_paths(isaaclab_settings: str) -> str:
    ...
    if os.path.exists(isaacsim_vscode_filename):
        ...
        # if the pattern is not found, return the original settings without modification
        if settings is None:
            settings = re.search(
                r"\"cursorpyright.analysis.extraPaths\": \[.*?\]", vscode_settings, flags=re.MULTILINE | re.DOTALL
            )
            if settings is None:
                raise ValueError("Could not find python.analysis.extraPaths in the Isaac Sim VSCode settings file.")
            else:
                settings = settings.group(0)
                settings = settings.split('"cursorpyright.analysis.extraPaths": [')[-1] 
    ...
    extraPaths_setting = '"python.analysis.extraPaths": [\n\t\t'.expandtabs(4) + path_names + "\n\t]".expandtabs(4)
    
    # Replace python.analysis.extraPaths
    ...
    # Also replace cursorpyright.analysis.extraPaths for Cursor IDE support
    cursorpyright_setting = '"cursorpyright.analysis.extraPaths": [\n\t\t'.expandtabs(4) + path_names + "\n\t]".expandtabs(4)
    isaaclab_settings = re.sub(
        r"\"cursorpyright.analysis.extraPaths\": \[.*?\]",
        cursorpyright_setting,
        isaaclab_settings,
        flags=re.DOTALL,
    )
